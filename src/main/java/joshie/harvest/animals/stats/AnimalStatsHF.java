...
public class AnimalStatsHF implements AnimalStats<NBTTagCompound> {
	...
	public boolean newDay() {
		if (animal != null) {
			...
			if (currentLifespan > type.getMaxLifespan()) {
				return false;
			}
	...
	public boolean performTest(AnimalTest test) {
		if (test == AnimalTest.HAS_EATEN) {
			return daysNotFed < 0;
		} else if (test == AnimalTest.IS_SICK) {
			return isSick;
		} else if (test == AnimalTest.HAD_TREAT) {
			return treated;
		} else if (test == AnimalTest.BEEN_LOVED) {
			return beenLoved;
		} else {
			return test == AnimalTest.WON_CONTEST && golden;
		}
	}
	...
	public boolean performAction(@Nonnull World world, @Nonnull ItemStack stack, AnimalAction action) {
		if (action == AnimalAction.FEED) {
			return feed(world);
		} else if (action == AnimalAction.HEAL) {
			return heal(world);
		} else if (action == AnimalAction.MAKE_GOLDEN) {
			return golden(world);
		} else if (action == AnimalAction.PETTED) {
			return pet(world);
		}
	...
	public void affectHappiness(int amount) {
		if (amount != 0) {
			...
			if (animal != null && !animal.world.isRemote) {
				if (amount < 0) {
					try {
						ReflectionHelper.findMethod(EntityLivingBase.class, "playHurtSound", "func_184581_c", DamageSource.class).invoke(
								animal,
								DamageSource.STARVE);
				...
				PacketHandler.sendToAllAround(
						new PacketSyncHappiness(animal.getEntityId(), amount),
						animal.dimension,
						animal.posX,
						animal.posY,
						animal.posZ,
