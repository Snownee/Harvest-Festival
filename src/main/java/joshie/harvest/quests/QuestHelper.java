...
public class QuestHelper implements IQuestHelper {
	...
	public void completeQuestConditionally(Quest quest, EntityPlayer player) {
		if (!hasCompleted(quest, player)) {
			if (quest.getQuestType() == TargetType.PLAYER) {
				HFTrackers.getPlayerTrackerFromPlayer(player).getQuests().markCompleted(player.world, player, quest, false);
			} else {
				TownHelper.getClosestTownToEntity(player, false).getQuests().markCompleted(player.world, player, quest, false);
			}
		}
	}
	...
	public void completeQuest(Quest quest, EntityPlayer player) {
		if (!player.world.isRemote) {
			if (quest.getQuestType() == TargetType.PLAYER) {
				HFTrackers.getPlayerTrackerFromPlayer(player).getQuests().markCompleted(player.world, player, quest, true);
			} else {
				TownHelper.getClosestTownToEntity(player, false).getQuests().markCompleted(player.world, player, quest, true);
			}
		}
	}
	...
	public boolean hasCompleted(Quest quest, EntityPlayer player) {
		...
		if (quest.getQuestType() == TargetType.PLAYER) {
			return HFTrackers.getPlayerTrackerFromPlayer(player).getQuests().getFinished().contains(quest);
		}
	...
	public static Quest getSelectiomFromID(EntityPlayer player, int selection) {
		...
		if (toFetch.getQuestType() == TargetType.PLAYER) {
			return HFTrackers.getPlayerTrackerFromPlayer(player).getQuests().getAQuest(toFetch);
		} else {
			return TownHelper.getClosestTownToEntity(player, false).getQuests().getAQuest(toFetch);
		}
	}
}