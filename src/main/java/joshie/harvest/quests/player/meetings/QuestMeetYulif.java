...
public class QuestMeetYulif extends Quest {
	...
	private String getWoodAmount() {
		if (HFQuests.LOGS_CARPENTER == 24) {
			return getLocalized("wood.dozentwo");
		} else if (HFQuests.LOGS_CARPENTER == 12) {
			return getLocalized("wood.dozen");
		} else if (HFQuests.LOGS_CARPENTER == 64) {
			return getLocalized("wood.stack");
		} else if (HFQuests.LOGS_CARPENTER == 32) {
			return getLocalized("wood.stackhalf");
		} else if (HFQuests.LOGS_CARPENTER == 16) {
			return getLocalized("wood.stackquarter");
		} else {
	...
	public ItemStack getCurrentIcon(World world, EntityPlayer player) {
		if (quest_stage == LOGS || quest_stage == FINISHED) {
			return GODDESS_STACK;
		} else if (quest_stage == SEED_CHAT) {
			return FLOWER_GIRL_STACK;
		} else {
	...
	public String getDescription(World world, EntityPlayer player) {
		if (quest_stage == WELCOME) {
			return getLocalized("description.info");
		} else if (quest_stage == LOGS) {
			return getLocalized("description.logs", getWoodAmount());
		} else if (quest_stage == SEED_CHAT) {
			return getLocalized("description.jade");
		} else if (quest_stage == FINISHED) {
			return getLocalized("description.goddess");
		} else {
			return super.getDescription(world, player);
		}
	}
