...
public class QuestDataServer extends QuestData {
	...
	public boolean startQuest(Quest q, boolean sync, @Nullable NBTTagCompound tag) {
		try {
			if (!finished.contains(q) || q.isRepeatable()) {
				Quest quest = q.getClass()
						.newInstance()
						.setRegistryName(q.getRegistryName())
						.setStage(0); //Set the current quest to your new
	...
	public void markCompleted(@Nonnull World world, @Nullable EntityPlayer player, Quest quest, boolean rewards) {
		...
		//Sync everything
		if ((quest.getQuestType() == TargetType.PLAYER || (quest.getQuestType() == TargetType.TOWN && rewards))) {
			master.sync(player, new PacketQuestCompleted(quest, rewards)); //Let this player claim the reward
		} else if (quest.getQuestType() == TargetType.TOWN) {
			master.sync(null, new PacketQuestCompleted(quest, false)); //Let the rest of the server know this was completed
		}
	...
	public void removeAsCurrent(@Nonnull World world, Quest quest) {
		...
		if (quest.getQuestType() == TargetType.TOWN) {
			master.sync(null, new PacketQuestRemove(quest)); //Let the rest of the server know this was completed
		}
		updateQuests(true); //Update the world on these quests, everytime one is completed
	}
	...
	private void updateQuests(Quest quest, boolean sync) {
		if (quest.getQuestType() != master.getTargetType()) {
			return; //If we aren't the same quest type, we don't get counted
		}
