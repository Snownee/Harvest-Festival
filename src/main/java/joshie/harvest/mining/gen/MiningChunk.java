...
public class MiningChunk implements IChunkGenerator {
	...
	public void setBlockState(ChunkPrimer primer, int x, int y, int z, IBlockState state, int chunkX) {
		...
		if (state.getBlock() == PORTAL.getBlock()) {
			...
			if (primer.getBlockState(x, y, z).getBlock() != Blocks.LADDER && primer.getBlockState(x, y, z).getBlock() !=
					PORTAL.getBlock()) {
				primer.setBlockState(x, y, z, FLOORS);
				if (primer.getBlockState(x, y + 1, z).getBlock() != Blocks.LADDER) {
					IBlockState theState = MiningRegistry.INSTANCE.getRandomStateForSeason(world, MiningHelper.getFloor(chunkX, y), season);
					if (theState != null) {
								...
								for (int x4 = -width; x4 <= width; x4++) {
									for (int z4 = -length; z4 < length; z4++) {
										for (EnumFacing enumFacing : EnumFacing.HORIZONTALS) {
											...
											if (x4 != z4 && getBlockState(primer, offset.getX(), offset.getY(), offset.getZ()) ==
													states[i][j] && rand.nextBoolean()) {
					...
					for (int i = 0; i < 16; i++) {
						for (int j = 0; j < 16; j++) {
							if (below[i][j] == LADDER) {
								...
								if (getBlockState(primer, i, belowY, j).getBlock() != LADDER.getBlock() && getBlockState(
										primer,
										i,
										belowY,
										j) != LADDER_HOLE) {
	...
	private void placePortals(ChunkPrimer primer, int mineID, int floor, int chunkX, int chunkY, int chunkZ) {
		if (isFloorWithPortal(floor) && !MineManager.areCoordinatesGenerated(world, mineID, floor)) {
			for (int x = 0; x < 16; x++) {
				for (int z = 0; z < 16; z++) {
	...
	private boolean isXLineWall(ChunkPrimer primer, int x, int y, int z) {
		return x + 1 < 16 && x - 1 >= 0 && getBlockState(primer, x, y, z) == WALLS && getBlockState(primer, x + 1, y, z) == WALLS &&
				getBlockState(primer, x - 1, y, z) == WALLS;
	}

	private boolean isXLineAir(ChunkPrimer primer, int x, int y, int z) {
		return (
				z + 1 < 16 && getBlockState(primer, x, y, z + 1) == AIR && getBlockState(primer, x + 1, y, z + 1) == AIR && getBlockState(
						primer,
						x - 1,
						y,
						z + 1) == AIR)
	...
	private boolean isZLineWall(ChunkPrimer primer, int x, int y, int z) {
		return z + 1 < 16 && z - 1 >= 0 && getBlockState(primer, x, y, z) == WALLS && getBlockState(primer, x, y, z + 1) == WALLS &&
				getBlockState(primer, x, y, z - 1) == WALLS;
	}
				...
				if (!(startX == -1 || endX == -1 || startZ == -1 || endZ == -1)) {
					...
					while (x != endX || z != endZ) {
						...
						for (EnumFacing enumfacing : EnumFacing.HORIZONTALS) {
							...
							if (pos.getX() < 0 || pos.getX() >= blockStateMap.length) {
								continue;
							}
							if (pos.getZ() < 0 || pos.getZ() >= blockStateMap[pos.getX()].length) {
								continue;
							}
								...
								for (int x4 = -wStart; x4 <= wEnd; x4++) {
									for (int z4 = -lStart; z4 <= lEnd; z4++) {
										if (x4 != z4 && blockStateMap[clamp(x2 + x4)][clamp(z2 + z4)] != LADDER) {
											...
											if ((differenceX >= ladderDistance || differenceZ >= ladderDistance) &&
													!MineManager.containsCoordinatesKey(mapIndex)) {
	...
	public BlockPos getNearestStructurePos(
			World worldIn, String structureName, BlockPos position,
